...
--- # Computer
---
--- This signal is queued by the [[block:case|computer]] or [[block:robot|robot]] when a new component is attached to it.
--- The address is the address of the added component,
--- `componentType` is the type of the component (e.g. `redstone` or `gpu`).
...
--- This includes left clicks in the GUI (i.e. when a keyboard is attached)
--- or when right-clicking/activating them in the world directly
--- (i.e. when no keyboard is attached or when sneak-activating).
--- The address is the address of the screen the queued the signal.
--- The x and y coordinates are in "letters" (meaning they map directly to `term.setCursor` or `gpu.set`, for example).
--- The player name is the user name of the player that triggered the event.
--- Note on the player name: I'll probably add an option to disable this argument in the future,
--- for those who think it's too... unrealistic.
...
--- The x and y coordinates are the cursor location when the scroll occurred and are,
--- like the `touch` signal, in "letters".
--- The `direction` indicates which way to scroll, where a positive value usually means "up",
--- whereas a negative value means "down".
...
--- The address is the address of the screen the queued the signal.
--- The x and y coordinates are *the coordinates of the sub-block* of the multi-block screen that queued the event.
--- Use [[component:gpu|gpu.getSize()]] to figure out which area of the display that actually represents.
...
--- more specifically when the user *presses* a key. This event may be repeated if the user keeps pressing the key.
...
--- on the screen it's attached to, more specifically when the user *releases* a key.
--- Note that although most cases where a player can be "removed" from a screen
--- without releasing the key before-hand *should* be handled (I think) there may still be cases where this fails.
--- Meaning this feature is more or less in an... observation stage,
--- and may be removed at a later point if it proves infeasible.
--- //Important//: this signal is *checked*, i.e. it is only queued on a computer
--- if the player that caused it is [[:computer_users|registered as a user]] on the computer
--- (or there are no users registered on the computer).
...
--- (Shift+Ins or middle mouse button).
--- Note that the maximum length of the text that can be pasted is limited (can be changed in the config).
--- //Important//: this signal is *checked*, i.e. it is only queued on a computer
--- if the player that caused it is [[:computer_users|registered as a user]] on the computer
--- (or there are no users registered on the computer).
...
--- The address is of the [[block:redstone_io|Redstone I/O block]] or that of the [[item:redstone_card|redstone card]]
--- installed in the machine where the redstone signal was detected.
--- The side is one of the [[api:sides|sides]] constants and indicates on which side the signal changed.
--- This is relative to the container of the component,
...
--- when they receive a message on an open port.
--- The first address is the address of the network card that received the message,
--- the second the address from where the message was sent.
--- Note that the sender address may differ from the card that originally sent the message when it passed
--- through one or more [[block:switch|switches]].
--- The port is the port on which the message was received.
--- This distance is the distance only set when receiving *wireless* network messages,
--- in which case it is the distance to the wireless network card that sent the message.
